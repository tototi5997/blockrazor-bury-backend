...
from app.schemas.page_view import PageViewListOut, PageViewOut, PageViewCreate, PageViewUpdate
...
def list_page_views(
    ...
    page_views = page_view_crud.get_page_views(db, page, page_size, visitor_id,
                                               ip_address, page_url)
    return Reponse(data=page_views, message='成功')


@router.post('/create', response_model=Reponse[PageViewOut])
def create_page_view(page_view: PageViewCreate,
                     request: Request,
                     db: Session = Depends(get_db)):
...
def update_page_view(view_id: int,
                     page_view: PageViewUpdate,
                     db: Session = Depends(get_db)):
    page_view = page_view_crud.update_page_view(db,
                                                page_view_id=view_id,
                                                page_view_in=page_view)
    if not page_view:
